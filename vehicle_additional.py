...
def get_location_metres(original_location, dNorth, dEast, is_bearing=False):
    """
        bearing = dNorth
        distance = dEast
    ...
    if is_bearing:
        ...
        dEast = math.sin(bearing * math.pi/180) * distance
        dNorth= math.cos(bearing * math.pi/180) * distance
...
def send_global_velocity(vehicle, velocity_x, velocity_y, velocity_z, duration=0):
    """
...
def vehicle_addi(v):
    v.arm_and_takeoff = MethodType(arm_and_takeoff, v)
    v.condition_yaw = MethodType(condition_yaw, v)
    v.goto_position_target_global_int = MethodType(
        goto_position_target_global_int, v)
    v.goto_position_target_local_ned = MethodType(
        goto_position_target_local_ned, v)
    v.send_global_velocity = MethodType(send_global_velocity, v)
    v.send_ned_velocity = MethodType(send_ned_velocity, v)
    v.set_roi = MethodType(set_roi, v)
